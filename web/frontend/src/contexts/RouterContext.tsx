...
export type Route = {
  path: 'home' | 'audio-detail' | 'settings' | 'chat' | 'global-chat'
...
export function RouterProvider({ children }: { children: React.ReactNode }) {
  ...
  const navigate = (route: Route) => {
    ...
    if (route.path === 'audio-detail' && route.params?.id) {
      ...
      url = '/settings'
    } else if (route.path === 'global-chat') {
      url = '/global-chat'
    }
      ...
      if (event.state?.route) {
        ...
        if (chatMatch) {
          ...
          if (chatBase) {
            ...
            return
          } else if (path === '/global-chat') {
            setCurrentRoute({ path: 'global-chat' })
