...
func NewHandler(cfg *config.Config, authService *auth.AuthService, taskQueue *queue.TaskQueue, unifiedProcessor *transcription.UnifiedJobProcessor, quickTranscription *transcription.QuickTranscriptionService, ragService *rag.RAGService) *Handler {
	return &Handler{
		config:              cfg,
...
func (h *Handler) UploadAudio(c *gin.Context) {
	...
	// Auto-transcribe: Get default profile or use system default
	var profile models.TranscriptionProfile
	var profileFound bool

	// Try to get user's default profile if authenticated
	if userID, exists := c.Get("user_id"); exists {
		var user models.User
		if err := database.DB.First(&user, userID).Error; err == nil && user.DefaultProfileID != nil {
	...
	// If we found a profile, update the job and queue it for auto-transcription
	if profileFound {
